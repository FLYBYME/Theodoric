...
function nextGenome() {
    ...
    const lastGenome = neat.population[neat._i - 1];
    if (lastGenome)
        console.log(`Genome score: ${lastGenome.score}`)

    if (neat._i >= neat.population.length) {
        console.log('reset');
        neat._i = 0;
        endEvaluation()
    }

    return neat.population[neat._i++]
}
function endEvaluation() {
    console.log('Generation:', neat.generation, '- average score:', neat.getAverage());

    neat.sort();
    drawGraph(neat.population[0].graph(200, 200), '.draw');
    localStorage.setItem('network', JSON.stringify(neat.export()))
...
Theodoric.Game.prototype = {
    ...
    update: function () {
        ...
        //console.log(g.game.physics.arcade.angleBetween(g.player, g.collectables.getClosestTo(g.player))+3)
        ...
        this.tickLabel.text = this.ticks + ' Ticks';
        this.tickMaxLabel.text = this.tickMax + ' TicksMax';
        
    },
    initNeat() {
        this.genome = nextGenome();
        this.genome.score = 0;
        this.distance = 0
        //console.log(this.genome)

    },
    generateInput(closest) {
        let inputs = [];
        //let closest = this.collectables.getClosestTo(this.player);
        let rad = this.game.physics.arcade.angleBetween(this.player, closest) + 3;
        inputs.push(this.game.math.clamp(this.game.math.mapLinear(rad, 0, 6, 0, 1), 0, 1))

        let distance = this.game.physics.arcade.distanceBetween(this.player, closest)
        inputs.push(this.game.math.clamp(this.game.math.mapLinear(distance, 0, 200, 0, 1), 0, 1))
        return inputs;
    },
    generateInputs() {
        let inputs = [];
        let collectable = this.generateInput(this.collectables.getClosestTo(this.player));
        inputs.push(...collectable)
        let obstacle = this.generateInput(this.obstacles.getClosestTo(this.player));
        inputs.push(...obstacle)
        //console.log(inputs, this.neat.population[0].activate(inputs))
        this.inputs = this.genome.activate(inputs)
        this.controls.up.isDown = this.inputs[0] > 0.5;
        this.controls.down.isDown = this.inputs[1] > 0.5;
        this.controls.left.isDown = this.inputs[2] > 0.5;
        this.controls.right.isDown = this.inputs[3] > 0.5;
    },
    ...
    collisionHandler: function () {
        ...
        this.game.physics.arcade.collide(this.obstacles, this.player, () => {
            this.tickMax -= 10;
        }, null, this);
    ...
    showLabels: function () {
        ...
        this.tickLabel = this.game.add.text(this.game.width - 75, this.game.height - 50, text, style);
        this.tickLabel.fixedToCamera = true;

        this.tickMaxLabel = this.game.add.text(this.game.width - 75, this.game.height - 75, text, style);
        this.tickMaxLabel.fixedToCamera = true;
        this.distanceLabel = this.game.add.text(this.game.width - 75, this.game.height - 100, text, style);
    ...
    deathHandler: function (target) {
        ...
        this.genome.score = this.tickMax;
        target.destroy();
    },
    ...
    playerMovementHandler: function () {
        ...
        // Up-Left
         if (this.controls.up.isDown && this.controls.left.isDown) {
            ...
            this.distance += this.player.speed / 2;
        ...
        this.distanceLabel.text = `${this.distance} distance`
    },
