...
export default class Neat extends EventEmitter {
    constructor(input, output, fitness, options) {
        ...
        this.mutationRate = options.mutationRate || 1;
        ...
        this.template = options.network || neataptic.architect.LSTM(
            input,
            1,
            1,
            1,
    ...
    readyToEvolve() {
        return this.popIndex + 1 > this.population.length;
    }
    next() {

        let next = this.population[this.popIndex++];
        if (!next) {
            console.log(`evolve: ${this.population.map((g) => g.score)} `)
            this.evolve();

            this.popIndex = 0;
            next = this.population[this.popIndex++];
        }
        return next;
    }
